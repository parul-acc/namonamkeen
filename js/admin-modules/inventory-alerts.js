...
export async function loadStockAlerts() {
    ...
    try {
        ...
        alertsSnapshot.forEach(d => {
            ...
            alertsHTML += `
                <div style="background:${bgColor}; border-left:4px solid ${textColor}; padding:12px; margin-bottom:10px; border-radius:6px; display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <button class="btn btn-sm" onclick="acknowledgeAlert('${docId}')" 
        ...
        if (dashboardAlert && dashboardList && dashboardItems.length > 0) {
            dashboardAlert.style.display = 'flex'; // Changed from block to flex to match CSS/Structure
...
export async function acknowledgeAlert(alertId) {
    ...
    try {
        const alertRef = doc(db, 'restockAlerts', alertId);
